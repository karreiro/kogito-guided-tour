...
export type EventLabel = "registerTutorial" | "startTutorial";
...
export class GuidedTourEventBus {
  ...
  registerTutorial(tutorial: Tutorial) {
    const event = this.createEvent("registerTutorial", tutorial);
    this.dispatchEvent(event);
  }

  startTutorial(tutorialLabel: string) {
    const event = this.createEvent("startTutorial", tutorialLabel);
    this.dispatchEvent(event);
  }
